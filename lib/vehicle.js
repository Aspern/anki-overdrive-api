 ...
 * the read/write characteristics, the vehicle tries to send an payload to activate the SDK
 ...
 * @param {Integer} (optional) acceleration acceleration, default is 1500 mm/sec²
...
Vehicle.prototype.setSpeed = function (speed, accel, respectRoadPieceSpeedLimit) {
    ...
    // Cars only accept speed by parameter. The don't detect speed changed to zero.
    if (this._data.speed) {
...
Vehicle.prototype.uTurnJumṕ = function (immediate) {
    return this.turn(VEHICLE_TURN_UTURN_JUMP, immediate);
}

/**
 * Defines the offsetFromRoadCenter from road center, where the vehicle should drive.
 *
 * @param {Float} offset Offset from road center in mm
 * @returns {Promise} State after the offsetFromRoadCenter is reached
 */
Vehicle.prototype.offsetFromRoadCenter = function (offset) {
    var msg = new Buffer(6);

    msg.writeUInt8(0x5, 0);
...
/**
 * Changes the lane of the vehicle using the offsetFromRoadCenter from road center. The speed and acceleration
...
/**
 * Registers a listener on offsetFromRoadCenter from road center updates for the vehicle.
 *
 * @param {Function} callback Listener for offsetFromRoadCenter from road center updates
 */
...
Vehicle.prototype.unsubscribe = function (listener) {
...
/**
 * Tries to send a payload to the vehicle using the GATT write characteristic of BLE. Returns an
 ...
 * @returns {Promise} State after successfully sending the payload
...
Vehicle.prototype.subscribe = function (listener) {
    this._read.on('data', listener);
};

/**
 * Searches for a payload with a specified id in the data event of the GATT read characteristic.
 * When a payload was found it will be parsed by a ´msgParser´ function and send back using a
 * callback.
 *
 * @private
 * @param {Number} messageId  The id of the payload
 * @param {Function} callback Callback function to handle the data
 * @param {Function} msgParser  Parser function to transform the payload to readable data
 * @param {Boolean} (otpional) unique Defines if the payload should be read once, default is false
...
/**
 * Parses the ping payload into a Integer value.
 *
 * @private
 * @param (Buffer) msg Raw ping payload
...
/**
 * Parses the version payload into a Integer value.
 *
 * @private
 * @param (Buffer) msg Raw version payload
...
/**
 * Parses the battery level payload into a Integer value.
 *
 * @private
 * @param (Buffer) msg Raw battery level payload
...
/**
 * Parses the position update payload into a JSON-Object.
 *
 * @private
 * @param {Buffer} msg Raw data of position update
 * @returns {Object} payload of position update
 */
...
/**
 * Parses the transition update payload into a JSON-Object.
 *
 * @private
 * @param {Buffer} msg Raw data of transition update
 * @returns {Object} payload of transition update
 */
...
/**
 * Parses the intersection update payload into a JSON-Object.
 *
 * @private
 * @param {Buffer} msg Raw data of intersection update
 * @returns {Object} payload of intersection update
 */
...
/**
 * Parses the offsetFromRoadCenter from road center update payload into a JSON-Object.
 *
 * @private
 * @param {Buffer} msg Raw data of intersection update
 * @returns {Object} payload of offsetFromRoadCenter from road center update
 */
...
/*
 ...
 This payload conveys some basic info about the vehicle, similar to what is in the advertisement packet, but this is sent once you are connected to a vehicle.
